...
const PROBLEM_DB = {
    "task1": {
        ...
        correctAnswer: "과제1: 시준이의 생각은 틀렸다. (A로 갈 확률은 1/2, B와 C로 갈 확률은 각각 1/4이다.) / 과제2: 공이 위치 B에 도착할 확률은 1/4 (또는 25%) 이다.",
...
시준: 공 A, B, C의 세 위치 중 하나에 떨어질 테니까 B 위치에 떨어질 확률은 1/3이야! </p>
    ...
    "task2": {
        ...
        correctAnswer: "과제1: 집에서 학교까지의 거리를 x km로 둔다. / 과제2: 시간=거리/속력이므로 'x/4 - x/12 = 1/3 (또는 20/60)' 방정식을 세우고 풀면 거리는 2km 이다.",
...
// 모둠원 참여 설정
let ACTIVE_STUDENTS = ["민준", "서연", "연우"];
const savedActiveStudents = localStorage.getItem('active_students');
if (savedActiveStudents) {
    ACTIVE_STUDENTS = JSON.parse(savedActiveStudents);
} else {
    const STUDENT_COUNT = parseInt(localStorage.getItem('student_count') || '3', 10);
    ACTIVE_STUDENTS = ["민준", "서연", "연우"].slice(0, STUDENT_COUNT);
}
...
function getSystemPrompt(roleName) {
    ...
    if (roleName === "교사") {
        prompt = `당신의 이름은 '교사'이며, 중학교 1학년 수학 교사로서 모둠 방 대화에 참여하고 있습니다. 당신은 '대화적 교수법(Dialogic Teaching)'을 실천하는 교사입니다. 학생들(${ACTIVE_STUDENTS.join(", ")})의 대화가 정체되거나, 현재 수행중인 과제를 해결했다고 판단될 때 자연스럽게 개입하세요. 반말이 아닌 친절하고 부드러운 존댓말(예: ~해요, ~할까요?)을 1~2문장으로 짧게 사용하세요.\n\n${TEACHER_PROMPT_BASE}\n\n다음 과제 배경과 교사용 정답을 참고하세요.\n${TASK_INFO}\n✅ [교사용 실제 정답 (학생에겐 비밀)]: ${currentTask.correctAnswer || "정답 정보 없음"}\n\n[🚨운영 지침: 당신은 과제 진행자입니다. 과제1부터 과제${currentTask.goals.length}까지 한 번에 하나씩 순차적으로 제시하세요. 학생들이 현재 과제의 '핵심 정답'을 완벽하게 도출하고 논리적으로 합의했다면, 즉시 칭찬하며 다음 과제로 넘어가세요. 만약 학생들이 마지막 과제까지 모든 정답을 정확히 찾았거나 올바르게 요약했다면, 반드시 "정답입니다! 오늘 모둠 활동은 여기까지 할게요! 모두 수고했어요."라고 말하며 대화를 완전히 마무리하세요. (단, 정답이 애매하거나, 찍어서 맞췄거나, 논리가 부족하거나, 마지막 발화에 오개념이 있다면 절대 종료하지 말고 집요하게 질문을 던지세요.) 또한 교사가 먼저 정답을 알려주지 말고 다음과 같이 유도하세요. ${currentTask.teacherHint}]\n`;
    } else {
...
🚨 [특별 지침: 수업 종료]: 만약 직전 대화에서 학생들이 이번 과제의 실제 정답(${currentTask.correctAnswer || "정답"})을 정확하고 논리적으로 찾아내어 올바르게 요약했다면 **더 이상 추가 질문이나 토론 유도를 절대 하지 말고** 즉시 "정답입니다! 오늘 모둠 활동은 여기까지 할게요! 모두 수고했어요."라고 말하며 토론을 완전히 종료하세요. (단, 정답이 애매하거나, 직전 학생의 발화에 엉뚱한 오개념이나 잘못된 일반화가 포함되어 있다면 절대 종료하지 말고 다시 대화적 원칙을 적용하여 집요하게 파고드세요!)
...
[현재 과제의 실제 정답]
${currentTask.correctAnswer || "정답 정보 없음"}
...
- 🚨 [수업 종료 감지] 학생들이 위의 [현재 과제의 실제 정답]을 완벽하게 맞추고 이해했음을 논리적으로 확인했을 때 (애매한 정답이나 찍기, 오개념이 섞여있다면 절대 종료 개입을 위한 YES를 내지 마세요.)
...
async function runAiTurnChain(lastSpeaker, lastMessageText = "") {
    ...
    while (keepGoing && chainCount < 100) {
        ...
        if (ACTIVE_STUDENTS.length === 1) {
            if (ACTIVE_STUDENTS.includes("민준")) probMinjun = 1.0;
            if (ACTIVE_STUDENTS.includes("서연")) probSeoyeon = 1.0;
            if (ACTIVE_STUDENTS.includes("연우")) probYeonwoo = 1.0;
        } else if (callMinjun || callSeoyeon || callYeonwoo) {
        ...
        } else if (ACTIVE_STUDENTS.length === 2 && currentSpeaker !== "교사") {
            // 학생이 2명일 때 직전 화자가 학생이라면, 나머지 1명이 100% 확률로 이어받음 (번갈아 가며 대화)
            probMinjun = (ACTIVE_STUDENTS.includes("민준") && currentSpeaker !== "민준") ? 1.0 : 0.0;
            probSeoyeon = (ACTIVE_STUDENTS.includes("서연") && currentSpeaker !== "서연") ? 1.0 : 0.0;
            probYeonwoo = (ACTIVE_STUDENTS.includes("연우") && currentSpeaker !== "연우") ? 1.0 : 0.0;
        }
